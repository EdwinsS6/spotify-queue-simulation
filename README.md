...
Este proyecto implementa una simulación tipo Spotify utilizando una estructura de datos Cola (FIFO) desarrollada desde cero en Java. Se trabajó con arquitectura modular usando Maven, separando la librería de la estructura de datos y el proyecto consumidor.

Para compilar la librería, ubicarse en la carpeta `data-structure.queue` y ejecutar: mvn clean package. Para instalarla en el repositorio local de Maven, ejecutar: mvn clean install. Esto permite que el proyecto handler la utilice como dependencia.
